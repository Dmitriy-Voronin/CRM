...
export function getContactIconType(contactType) {
  const contactTypes = {
    'Телефон': `<svg id="phone" width="16" height="16" viewBox="0 0 16 16" fill="none"
    ...
    'E-mail': `<svg id="email" width="16" height="16" viewBox="0 0 16 16" fill="none"
              xmlns="http://www.w3.org/2000/svg">
    ...
    'Facebook': `<svg id="fb" width="16" height="16" viewBox="0 0 16 16" fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <g opacity="0.7">
              <path
    ...
    'Vk': `<svg id="vk" width="16" height="16" viewBox="0 0 16 16" fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <g opacity="0.7">
              <path
  ...
  return contactTypes[contactType] || `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path opacity="0.7" fill-rule="evenodd" clip-rule="evenodd"
...
export function createContactItemByType(contactType, contactValue) {
  const contactIcon = document.createElement('button');
  contactIcon.classList.add('btn-reset', 'btn', 'js-tooltip', 'contact-btn');
  contactIcon.dataset.type = `${contactType}`
  contactIcon.dataset.value = `${contactValue}`;
  contactIcon.innerHTML = getContactIconType(contactType);
  if (contactType === 'Телефон' || contactType === 'Доп. телефон') {
    tippy(contactIcon, {
      trigger: 'mouseenter',
      duration: 200,
      allowHTML: true,
      content: `<span style = 'font-weight: 400'>${contactType}</span>: <a href="tel:${contactValue}">${contactValue}</a>`,
      // maxWidth: 264,
      interactive: true,
    });
    ...
    tippy(contactIcon, {
      ...
      duration: 200,
      allowHTML: true,
      content: `<span style = 'font-weight: 400'>${contactType}</span>: <a href="mailto:${contactValue}">${contactValue}</a>`,
      // maxWidth: 264,
      interactive: true,
    });
  } else {
    tippy(contactIcon, {
      ...
      duration: 200,
      allowHTML: true,
      content: `<span style = 'font-weight: 400'>${contactType}</span>: <a href="${contactValue}">${contactValue}</a>`,
      // maxWidth: 264,
      interactive: true,
      });
  }
...
export function getValidateString(string) {
